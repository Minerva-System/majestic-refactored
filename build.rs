...
fn get_git_version() -> Result<String, Box<dyn Error>> {
    ...
    let branch_name = Command::new("git")
        ...
        .arg("--show-current")
        .output()?;
    let branch_name = str::from_utf8(&branch_name.stdout)?.trim_end();
    ...
    let latest_tag = Command::new("git")
        ...
        .arg("--abbrev=0")
        .output()?;
    let latest_tag = str::from_utf8(&latest_tag.stdout)?.trim_end();
    ...
    let commit_short = Command::new("git")
        ...
        .arg("HEAD")
        .output()?;
    let commit_short = str::from_utf8(&commit_short.stdout)?.trim_end();
    ...
    let tag_short = Command::new("git")
        ...
        .arg(latest_tag)
        .output()?;
    let tag_short = str::from_utf8(&tag_short.stdout)?.trim_end();

    if tag_short == commit_short {
        Ok(String::new()) // Favor version at Cargo.toml
    } else {
        Ok(format!("{}-{}", commit_short, branch_name))
    }
}
